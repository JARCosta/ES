...
public class DemoService {
    ...
    // This is an example of how we can change the state associated with the demo teacher (this method is in DemoService.java; you will need to also update DemoUtils.java to execute this method with the profile test-int).
    // It is also possible to add students, questions, and quizzes here.
    // IMPORTANT NOTE: some existing reset methods and additional reset methods will need to be updated/added (for example, to delete the newly created course executions). Otherwise, the database needs to be cleaned before each TutorApplication execution.
